...
namespace Awesome_Time.Tests.ServiceTests
{
    ...
    class ArticleServiceTests
    {
        ...
        [Test]
        public void GetLatestArticlePreviewsTest()
        {
            var latestArticleTitle = "One T";
            var latestArticleHtmlBody = "<p>GGGGG</p>";
            var latestArticleNonHtmlBody = "GGGGG...";
            #region Arrange data
            var data = new List<Article>
            {
                new Article
            ...
            var service = new ArticleService(mockDbContext.Object);
            var articlecount = 2;
            var result = service.GetLatestArticlePreviews(articlecount);

            // Assert
            Assert.AreEqual(result.Count, articlecount);
            Assert.AreEqual(result[0].Title, latestArticleTitle);
            Assert.AreEqual(result[0].Body, latestArticleNonHtmlBody);
        }
    }
}
